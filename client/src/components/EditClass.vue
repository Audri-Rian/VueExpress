    ...
    watch: {
        'form.course': {
            handler(newCourseId) {
                ...
                // Log para debug
                console.log('Curso selecionado:', this.courses.find(c => c._id === newCourseId));
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                <div class="bg-white/80 backdrop-blur-sm rounded-xl shadow-lg p-6">
                    ...
                    <form @submit.prevent="saveClass" class="space-y-6">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div class="space-y-2">
                                <label class="block text-sm font-semibold text-gray-700">Nome da Turma <span class="text-red-500">*</span></label>
                                <input ref="nomeInput" v-model="form.name" type="text" placeholder="Ex: Turma A, Turma Manhã..."
                            ...
                            <div class="space-y-2">
                                <label class="block text-sm font-semibold text-gray-700">Período <span class="text-red-500">*</span></label>
                                <select v-model="form.period" class="w-full bg-white/70 border-2 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all duration-200 appearance-none"
                                    :class="{ 'border-red-300 focus:border-red-500 focus:ring-red-500/20': errors.period, 'border-gray-200': !errors.period }">
                                ...
                                <p v-if="form.course && availablePeriods.length === 0" class="text-yellow-500 text-xs">
                                    Nenhum período disponível para este curso
                                </p>
                            </div>

                            <div class="space-y-2">
                                <label class="block text-sm font-semibold text-gray-700">Ano Letivo <span class="text-red-500">*</span></label>
                                <input v-model.number="form.year" type="number" :min="2000" :max="currentYear + 5" 
                                    placeholder="2025" class="w-full bg-white/70 border-2 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all duration-200"
                                    :class="{ 'border-red-300 focus:border-red-500 focus:ring-red-500/20': errors.year, 'border-gray-200': !errors.year }">
                                <p v-if="errors.year" class="text-red-500 text-xs">Ano deve estar entre 2000 e {{ currentYear + 5 }}</p>
                            </div>

                            <div class="space-y-2">
                                <label class="block text-sm font-semibold text-gray-700">Turno <span class="text-red-500">*</span></label>
                                <select v-model="form.shift" class="w-full bg-white/70 border-2 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all duration-200 appearance-none"
                                    :class="{ 'border-red-300 focus:border-red-500 focus:ring-red-500/20': errors.shift, 'border-gray-200': !errors.shift }">
                ...
                <div class="bg-white/80 backdrop-blur-sm rounded-xl shadow-lg p-6">
                    ...
                    <div v-else class="space-y-4">
                        <div v-for="turma in filteredClasses" :key="turma._id"
                            ...
                            <div class="flex items-start justify-between">
                                <div class="flex-1 min-w-0">
                                    ...
                                    <div class="flex items-center mt-1 space-x-2">
                                        <span v-if="turma.course" class="text-xs bg-blue-100 text-blue-800 px-2 py-1 rounded">{{ turma.course.title }}</span>
                                        <span v-else class="text-xs bg-gray-100 text-gray-800 px-2 py-1 rounded">Curso não definido</span>
                                ...
                                <div class="flex space-x-2 ml-4 flex-shrink-0">
                                    <button @click="editClass(turma._id)" class="p-1 text-gray-400 hover:text-blue-600 transition-colors">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path>
                                        </svg>
                                    </button>
                                    <button @click="deleteClass(turma._id)" class="p-1 text-gray-400 hover:text-red-600 transition-colors">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div class="bg-white/95 backdrop-blur-sm rounded-3xl shadow-2xl w-full max-w-md border border-white/30 transform scale-100 transition-all duration-300">
                    ...
                    <div class="p-8">
                        ...
                        <div class="flex flex-col sm:flex-row gap-3">
                            ...
                            <button @click="deleteClass(deletingIndex)" 
