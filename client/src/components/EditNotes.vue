          ...
          <div class="space-y-2">
            ...
            <div class="relative">
              <select v-model="selectedDiscipline" class="w-full bg-white/70 border-2 rounded-xl px-4 py-3 pr-10 focus:ring-4 focus:ring-blue-500/20 focus:border-blue-500 transition-all duration-200 appearance-none">
                ...
                <option v-for="disc in disciplines" :key="disc.id" :value="disc.id">{{ disc.nome }}</option>
              </select>
              <div class="absolute inset-y-0 right-0 flex items-center pr-3 pointer-events-none">
                <svg class="w-5 h-5 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path>
...
export default {
  ...
  watch: {
    selectedCourse(novo) {
      ...
      this.disciplines = this.allDisciplines.filter(d => d.cursoId === novo);
    }
  },
  computed: {
    allDisciplines() {
  ...
  methods: {
    loadStudents() {
      ...
      this.grades = {};
      this.students.forEach(s => { this.grades[s.id] = this.grades[s.id] || null; });
    },
    saveGrades() {
